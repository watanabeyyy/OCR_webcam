...
def batch_norm_wrapper(inputs, phase_train=None, decay=0.99):
    ...
    axes = list(range(rank - 1))  # nn:[0], conv:[0,1,2]
...
x = tf.placeholder(tf.float32, shape=[None, 108,108,1],name= "input")
...
if is_training:
    ...
    for epoch in range(num_epoch):
        ...
        for idx in range(0, num_data, batch_size):
            ...
            _,loss=sess.run([train_step,cross_entropy],feed_dict={phase_train:True,x: batch_x, y_: batch_y, keep_prob: 0.5})
            print("epoch %d, images %d, loss %g" % (epoch,idx,loss/batch_size))
        # test_accuracy, test_loss = sess.run([accuracy,cross_entropy],feed_dict={phase_train:False,x: x_test, y_: y_test, keep_prob: 1.0})
        # # ファイルオープン
        # f = open('output.csv', 'a')
...
if __name__=="__main__":
    if is_training==False:
        import cv2
        im = np.zeros((4,108,108,1))
        temp = cv2.imread('./img/1/20171127_1873_223384.png',cv2.IMREAD_GRAYSCALE)
        temp = temp[10:-10,10:-10]
        im[0,:,:,0] =temp
        temp = cv2.imread('./img/5/20171127_1891_805643.png',cv2.IMREAD_GRAYSCALE)
        temp = temp[10:-10,10:-10]
        im[1,:,:,0] =temp
        temp = cv2.imread('./img/8/20171127_18114_509135.png',cv2.IMREAD_GRAYSCALE)
        temp = temp[10:-10,10:-10]
        im[2,:,:,0] =temp
        temp = cv2.imread('./img/0/20171127_1873_30895.png',cv2.IMREAD_GRAYSCALE)
        temp = temp[10:-10,10:-10]
        im[3,:,:,0] =temp
        error , num = pred(im)
        print(error)
